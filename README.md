...
第一版记录设置保存、终端打开/关闭、SFTP 会话和传输、Redis 测试连接和 key 扫描、数据库测试连接、SQL 执行和表数据加载等关键入口。默认不记录完整 SQL；只有开启 `logging.include_sql` 后，数据库查询日志才会包含完整 SQL。日志不会记录密码、私钥口令、Redis 密码、数据库密码或私钥内容。
