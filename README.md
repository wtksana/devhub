...
项目处于 MVP 阶段，核心 SSH 和 SFTP 能力已经进入可用状态。Redis 已支持连接配置、PING 测试、key 浏览、key 内容查看、`string` 编辑、TTL 管理、删除和重命名等基础管理能力；数据库、Docker、跳板机、SSH tunnel 和 AI 功能暂不在当前版本范围内。
...
- Redis 连接配置保存、测试连接、key 浏览、key 内容查看、`string` 编辑、TTL 管理、删除和重命名。
