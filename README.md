...
项目处于 MVP 阶段，核心 SSH 和 SFTP 能力已经进入可用状态。Redis 已支持连接配置、PING 测试和 key 浏览第一批能力；数据库、Docker、跳板机、SSH tunnel 和 AI 功能暂不在当前版本范围内。
...
- Redis：Rust 后端基于 `redis-rs` 测试连接和扫描 key
- 本地终端：`portable-pty`
- 本地数据库：`rusqlite`
- 剪贴板和文件选择：Tauri 插件
...
暂不支持 Redis key 内容查看/编辑、删除、TTL 修改、发布订阅或监控面板。
...
1. Redis key 内容查看、编辑、删除和 TTL 管理。
