...
项目处于 MVP 阶段，核心 SSH 和 SFTP 能力已经进入可用状态。Redis 已支持连接配置、PING 测试、key 浏览、新建 key、key 内容查看、`string` 和集合类型编辑、TTL 管理、批量操作、删除和重命名等基础管理能力。数据库已支持 MySQL / PostgreSQL 直连、连接测试、对象树、SQL 查询、SQL 文件、危险 SQL 确认、结果表格和基础表数据编辑；Docker、跳板机、SSH tunnel 和 AI 功能暂不在当前版本范围内。
