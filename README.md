...
项目处于 MVP 阶段，核心 SSH 和 SFTP 能力已经进入可用状态。Redis 已支持连接配置、PING 测试、key 浏览、新建 key、key 内容查看、`string` 和集合类型编辑、TTL 管理、批量操作、删除和重命名等基础管理能力。数据库 A 版已支持 MySQL / PostgreSQL 直连、连接测试、对象树、SQL 查询、危险 SQL 确认、结果表格和查询历史；Docker、跳板机、SSH tunnel 和 AI 功能暂不在当前版本范围内。
...
- MySQL / PostgreSQL 连接配置保存、测试连接、对象树、SQL 执行、结果表格、双击表查询和查询历史。
...
- 右侧 SQL 编辑器执行 SQL，默认 SELECT 自动追加 `LIMIT 200`。
...
- 双击表或视图会填入并执行 `SELECT * FROM <table> LIMIT 200`。
- 查询历史保存到本地 SQLite，每个连接保留最近 100 条。
- 查询历史面板支持查看最近 SQL，并点击回填到 SQL 编辑器。
...
- 文件大小显示方式。
- 连接配置。
- 查询历史保存在本地 SQLite，不写入 `settings.json`。
- 空分组列表。
...
1. 数据库 B 版：表数据分页筛选、只读/可写模式、表数据编辑。
2. Redis 发布订阅和监控面板。
3. SSH tunnel。
4. 跳板机。
5. SSH agent。
