...
前端整体体验参考 Zed，但不是复制 Zed 编辑器功能。第一版借鉴的是它的工作台结构、命令优先交互、紧凑面板、JSON 配置和键盘友好体验。

设计原则：

- 工作台采用左侧 dock、中央 editor/workspace、右侧 assistant 面板、底部状态栏的结构。
- 面板可以折叠，宽度可调整，并把宽度和可见状态写入 `settings.json`。
- 顶部不做厚重导航栏，主要命令通过命令面板、快捷键和工具按钮触发。
- UI 密度偏开发工具，不做营销式大卡片和大标题。
- 终端、SFTP、Settings、AI 都以标签页或 pane 的形式进入中央工作区。
- AI 面板在右侧保持上下文感知，但默认不遮挡终端和 SFTP 主操作区。
- 主题、字体、布局和快捷键都通过 Settings 面板和 JSON 配置控制。

第一版需要提供的基础前端组件：

- `AppShell`：整体工作台骨架。
- `DockPanel`：左侧连接 dock 和右侧 AI dock。
- `WorkspaceTabs`：中央标签栏。
- `StatusBar`：连接状态、任务状态、AI Provider 状态。
- `CommandPalette`：打开连接、打开 Settings、切换面板等基础命令入口。
...
- `AppShell`：Zed 风格工作台布局，组织 dock、workspace、AI panel 和 status bar。
- `CommandPalette`：命令入口和键盘优先操作。
- `WorkspaceTabs`：终端、SFTP、Settings 的标签管理。
