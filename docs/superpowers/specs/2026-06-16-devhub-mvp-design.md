...
- 第一版关闭 SFTP 标签会关闭后端 SFTP 会话；传输任务取消、暂停和恢复后续补齐。
...
6. 用户关闭 SFTP 标签时，前端调用后端关闭 SFTP 会话；传输任务取消能力后续补齐。
...
- 能用密码、私钥、带口令私钥连接 SSH。
- 能在终端中正常处理 sudo 交互。
- 能通过 SFTP 浏览目录、进入文件夹、后退、前进、刷新、上传、下载、上传文件夹、下载文件夹、拖拽上传、删除、重命名、新建目录、新建文件、压缩、解压、远程文本编辑和复制路径。
